...
// 2. Formatter un texte pour l'affichage
...
// Sélection brouillon (avant appliquer)
...
const filteredProducts = computed(() => {
  ...
  switch (sortBy.value) {
    case 'Par tarif croissant':  sorted.sort((a, b) => a.price - b.price); break
    case 'Par tarif decroissant': sorted.sort((a, b) => b.price - a.price); break
    case 'Par popularité':       sorted.sort((a, b) => b.popularity - a.popularity); break
  ...
  <UContainer class="py-8 bg-white">
      ...
      <Transition name="slide-up">
        ...
        >
            ...
            <div v-for="(options, key) in filterGroups" :key="key" class="border-b border-gray-50 pb-3">
              ...
              <div v-if="expandedGroups[key as string]" class="grid grid-cols-2 gap-1.5 pt-1">
                <label
                  ...
                  :key="opt"
                  class="flex items-center gap-2 px-3 py-2 rounded-sm border cursor-pointer transition-all text-xs font-medium"
          ...
          <div class="flex-shrink-0 px-4 py-4 border-t border-gray-100 bg-white">
            <UButton @click="applyFilter" color="error" block size="lg" class="rounded-sm font-bold">
    ...
    <div class="grid grid-cols-12 gap-8">
      ...
      <aside class="hidden lg:block col-span-3 space-y-8 flex-shrink-0">
        ...
        <!-- Groupes filtres desktop — pills radio -->
        ...
        <!-- Boutons Appliquer / Reset desktop -->
        ...
        <div>
          ...
          <UCarousel
            ...
            :prev="{ variant: 'solid' }"
            :next="{ variant: 'solid' }"
            :ui="{ prev: 'sm:start-2', next: 'sm:end-2' }"
          >
            ...
            <div v-for="service in servicesList" :key="service.key">
              ...
              <div v-show="expandedServices[service.key]" class="bg-gray-50/50 border-t border-gray-50 px-10 py-2 space-y-2">
                <NuxtLink
                  ...
                  :key="link.label"
                  :to="link.to"
                  class="block text-[12px] text-gray-600 hover:text-[#e60012] transition-colors"
                >
                  {{ link.label }}
                </NuxtLink>
              </div>
      ...
      <main class="col-span-12 lg:col-span-9">
        ...
        <div class="flex flex-wrap items-center justify-between mb-6 bg-[#f8f8f8] p-2 rounded-xl border border-gray-200 gap-4">
          <div class="flex items-center gap-3 ml-2">
            <button @click="viewMode = 'grid'" :class="viewMode === 'grid' ? 'text-black' : 'text-gray-300'">
              <UIcon name="i-heroicons-squares-2x2-solid" class="w-6 h-6" />
            </button>
            <button @click="viewMode = 'grid-small'" :class="viewMode === 'grid-small' ? 'text-black' : 'text-gray-300'">
              <UIcon name="i-heroicons-squares-plus-solid" class="w-6 h-6" />
            </button>
            <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'text-black' : 'text-gray-300'">
              <UIcon name="i-heroicons-bars-3-bottom-left-solid" class="w-6 h-6" />
            </button>
            <button @click="viewMode = 'list-compact'" :class="viewMode === 'list-compact' ? 'text-black' : 'text-gray-300'">
          ...
          <div class="flex items-center gap-4">
            <span class="hidden sm:block text-[13px] text-gray-500 font-medium">
              Affichage de
              <span class="text-gray-900 font-bold">{{ ((currentPage - 1) * itemsPerPage) + 1 }}</span>–<span class="text-gray-900 font-bold">{{ Math.min(currentPage * itemsPerPage, filteredProducts.length) }}</span>
              sur <span class="text-gray-900 font-bold">{{ filteredProducts.length }}</span> résultats
            </span>
        ...
        <div :class="{
          ...
          >
            ...
            <div
              :class="{
                'relative h-40 sm:h-48 md:h-52 w-full': viewMode === 'grid',
                'relative h-32 sm:h-40 w-full': viewMode === 'grid-small',
                'relative h-40 w-40': viewMode === 'list',
                'relative h-20 w-20': viewMode === 'list-compact'
              }"
              class="overflow-hidden bg-[#fcfcfc] flex-shrink-0"
            >
              <div class="absolute top-2 left-2 z-10 bg-[#e60012] text-white text-[10px] font-black px-1.5 py-0.5 rounded-sm">
                {{ p.discount }}
              </div>

              <div
                v-if="viewMode.startsWith('grid')"
                class="absolute right-2 top-2 flex flex-col gap-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
              >
                <button @click.prevent.stop="addToWishlist(p.id)" class="w-7 h-7 bg-white/90 shadow-md rounded-full flex items-center justify-center text-gray-400 hover:bg-[#e60012] hover:text-white transition-colors">
                  <UIcon name="i-heroicons-heart" class="w-3.5 h-3.5" />
                </button>
                <button @click.prevent.stop="" class="w-7 h-7 bg-white/90 shadow-md rounded-full flex items-center justify-center text-gray-400 hover:bg-[#003366] hover:text-white transition-colors">
              ...
              <!-- Bouton panier slide-up desktop uniquement -->
              <div v-if="viewMode.startsWith('grid')" class="hidden sm:block absolute bottom-[-100%] group-hover:bottom-0 left-0 w-full z-20 transition-all duration-300">
                <UButton @click.prevent.stop="addToCart(p.id)" icon="i-heroicons-shopping-cart" block class="bg-[#274a82] hover:bg-[#e60012] rounded-none font-bold text-[12px] py-3">
                  Ajouter au panier
                </UButton>
              </div>
            </div>
            ...
            <!-- INFOS -->
            <div
              class="p-2 sm:p-3 flex flex-col flex-1 border-t border-gray-50"
              :class="viewMode === 'list-compact' ? 'p-1 border-t-0' : ''"
            >
              <span class="text-[11px] text-gray-400 font-bold tracking-widest">{{ p.category }}</span>

              <h3
                class="text-gray-600 font-medium line-clamp-2 leading-snug group-hover:underline"
                :class="viewMode === 'list-compact' ? 'text-[12px] h-4 mb-0' : 'text-sm sm:text-[15px] h-[40px] sm:h-[44px] mb-2 overflow-hidden'"
              >
                {{ p.name }}
              </h3>

              <!-- Desktop: prix simple (grid) -->
              <div v-if="viewMode.startsWith('grid')" class="hidden sm:block mt-auto">
                <div class="text-2xl font-black text-gray-900 mb-0.5">
                  {{ p.price.toLocaleString() }} <span class="text-[11px] font-semibold">FCFA</span>
                </div>
                <span class="text-[12px] text-gray-400 line-through">{{ p.oldPrice.toLocaleString() }} FCFA</span>
              </div>

              <!-- Mobile: prix + bouton panier rond (grid) -->
              <div v-if="viewMode.startsWith('grid')" class="sm:hidden mt-auto flex items-center justify-between gap-1">
                <div>
                  <div class="text-xl font-black text-gray-900 leading-tight">
                    {{ p.price.toLocaleString() }} <span class="text-[9px] font-semibold">FCFA</span>
                  </div>
                  <span class="text-[10px] text-gray-400 line-through">{{ p.oldPrice.toLocaleString() }} FCFA</span>
                </div>
                <button @click.prevent.stop="addToCart(p.id)" class="flex-shrink-0 w-8 h-8 rounded-full bg-[#274a82] hover:bg-[#e60012] flex items-center justify-center text-white transition-colors shadow-sm">
                  <UIcon name="i-heroicons-shopping-cart" class="w-4 h-4" />
                </button>
              </div>

              <!-- Mode list: prix + actions -->
              <div v-if="viewMode.startsWith('list')" class="mt-auto flex items-end justify-between">
                <div>
            ...
            >
              <div class="relative h-64 w-full overflow-hidden p-2 flex items-center justify-center">
                ...
                <div class="absolute bottom-[-100%] group-hover:bottom-0 left-0 w-full transition-all duration-300 z-20">
                  <UButton @click.prevent.stop="addToCart(p.id)" icon="i-heroicons-shopping-cart" block class="bg-[#274a82] rounded-none font-bold text-[12px] py-2.5">
                    Ajouter au Panier
                  </UButton>
                </div>
              </div>
        ...
        <!-- MOBILE ONLY : Promos + Meilleur Produits + Expertises -->
