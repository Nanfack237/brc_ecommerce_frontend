...
useSeoMeta({
  title:   t('cart_page.seo_title'),
  ogTitle: t('cart_page.seo_og_title'),
...
const breadcrumbItems = computed(() => [
  { label: t('cart_page.breadcrumb_home'), to: '/' },
  { label: t('cart_page.breadcrumb_cart'), to: '/cart', current: true },
])

const articleLabel = computed(() =>
  itemCount.value > 1 ? t('cart_page.articles') : t('cart_page.article')
)
...
const handleDecrease = (item: any) => {
  ...
  item.quantity--
  try { updateQuantity(item.id, item.quantity) } catch {}
}
...
const handleClear = () => {
  try { clearCart() } catch { cartItems.value = [] }
}
</script>
  ...
  <UContainer class="py-6 bg-white min-h-screen">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <h1 class="text-2xl font-bold text-gray-800 tracking-tight">{{ $t('cart_page.title') }}</h1>
    ...
    <div v-if="cartItems.length === 0" class="flex flex-col items-center justify-center py-24 gap-6 text-center">
      ...
      <div>
        <p class="text-xl font-bold text-gray-700 mb-1">{{ $t('cart_page.empty_title') }}</p>
        <p class="text-sm text-gray-400">{{ $t('cart_page.empty_sub') }}</p>
      </div>
        ...
        <div v-for="item in cartItems" :key="item.id"
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-xs text-gray-400 font-medium">
              {{ $t('cart_page.unit_price') }}
              <span class="text-gray-600 font-bold">{{ formatPrice(item.price) }} {{ $t('cart_page.currency') }}</span>
            </p>
          </div>
          ...
          <div class="flex flex-col items-end gap-3 flex-shrink-0">
            ...
            <p class="font-black text-[#e60012] text-base sm:text-lg whitespace-nowrap">
              ...
              <span class="text-[10px] font-medium text-gray-400"> {{ $t('cart_page.currency') }}</span>
            </p>
      ...
      <div class="lg:col-span-1">
        <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden sticky top-6">
          ...
          <div class="bg-[#274a82] px-5 py-4">
            <h2 class="font-black text-white text-base tracking-wide">{{ $t('cart_page.summary_title') }}</h2>
            <p class="text-white/60 text-[11px] mt-0.5">{{ itemCount }} {{ articleLabel }}</p>
          </div>
          ...
          <div class="px-5 py-4 space-y-3">
            ...
            <div class="pt-1">
              <div class="flex justify-between items-center">
                <span class="font-black text-gray-900 text-base">{{ $t('cart_page.total') }}</span>
                <div class="text-right">
                  <p class="font-black text-[#e60012] text-xl leading-tight">{{ formatPrice(total) }}</p>
                  <p class="text-[10px] text-gray-400 font-medium">{{ $t('cart_page.currency_ttc') }}</p>
                </div>
              </div>
            </div>
